 ...
 */
public class ItemNSC2A extends ItemMinecart {

    public ItemNSC2A() {
        super(-1);
        setUnlocalizedName("ItemNSC2A");
        setTexName("item_nsc_2a");
        setMaxStackSize(64);
    ...
    public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side, float px, float py, float pz) {
        ...
        {
            if (!world.isRemote) {
                NSC2A cart = new NSC2A(world, (double) x + 0.5, (double) y + 0.5, (double) z + 0.5);
