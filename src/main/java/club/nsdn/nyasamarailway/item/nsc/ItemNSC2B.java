 ...
 */
public class ItemNSC2B extends ItemMinecart {

    public ItemNSC2B() {
        super(-1);
        setUnlocalizedName("ItemNSC2B");
        setTexName("item_nsc_2b");
        setMaxStackSize(64);
    ...
    public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side, float px, float py, float pz) {
        if (BlockRailBase.func_150051_a(world.getBlock(x, y, z)) && world.getBlock(x, y, z) instanceof RailMonoMagnetBase)
        {
            if (!world.isRemote)
            {
                MinecartBase entityminecart = new NSC2B(world, (double) x + 0.5, (double) y + 0.5, (double) z + 0.5);
