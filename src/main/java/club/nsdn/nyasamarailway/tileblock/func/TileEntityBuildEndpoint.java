    ...
    void updateRoute() {
        ...
        while (true) {
            if (te instanceof TileEntityActuator) {
                ...
                points.add(new Vec3d(actuator.getPos()).addVector(offset, offset, offset));
        ...
        if (
                Math.abs(points.peekLast().x - points.peekFirst().x)
                >
                Math.abs(points.peekLast().z - points.peekFirst().z)
        ) {
            inv = false;
