 ...
 */
public class BlockGateBase extends TileBlock {
    ...
    public static class GateBase extends TileEntity {
        ...
        public boolean isDoor(int dx, int dz) {
            ...
            ) instanceof BlockGateDoor.GateDoor;
        }

        public int getDoorState(int dx, int dz) {
            return ((BlockGateDoor.GateDoor) (worldObj.getTileEntity(
                    xCoord + dx, yCoord, zCoord + dz
            ))).state;
        }
        ...
        public int getDoorState() {
            ...
            return BlockGateDoor.GateDoor.STATE_CLOSE;
        }

        public void setDoorState(int dx, int dz, int state) {
            ((BlockGateDoor.GateDoor) (worldObj.getTileEntity(
            xCoord + dx, yCoord, zCoord + dz
            ))).state = state;
        }
        ...
        public void openDoor() {
            ...
            switch (direction) {
                case SOUTH:
                    if (isDoor(1, 0)) {
                        if (getDoorState(1, 0) == BlockGateDoor.GateDoor.STATE_CLOSE) {
                            setDoorState(1, 0, BlockGateDoor.GateDoor.STATE_OPENING);
                        }
                    }
                    break;
                case WEST:
                    if (isDoor(0, 1)) {
                        if (getDoorState(0, 1) == BlockGateDoor.GateDoor.STATE_CLOSE) {
                            setDoorState(0, 1, BlockGateDoor.GateDoor.STATE_OPENING);
                        }
                    }
                    break;
                case NORTH:
        ...
        public void closeDoor() {
            ...
            switch (direction) {
                case SOUTH:
                    if (isDoor(1, 0)) {
                        if (getDoorState(1, 0) == BlockGateDoor.GateDoor.STATE_OPEN) {
                            setDoorState(1, 0, BlockGateDoor.GateDoor.STATE_CLOSING);
                        }
                    }
                    break;
                case WEST:
                    if (isDoor(0, 1)) {
                        if (getDoorState(0, 1) == BlockGateDoor.GateDoor.STATE_OPEN) {
                            setDoorState(0, 1, BlockGateDoor.GateDoor.STATE_CLOSING);
                        }
                    }
                    break;
                case NORTH:
