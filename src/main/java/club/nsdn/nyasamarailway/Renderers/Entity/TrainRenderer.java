 ...
 */
public class TrainRenderer extends RenderMinecart {
    private ResourceLocation trainTex[];
    public ModelBase modelTrain[];

    public TrainRenderer(ModelBase model[], String texturePath[]) {
        super();
    ...
    public void doRender(EntityMinecart minecart, double x, double y, double z, float Yaw, float p_doRender_9_) {
        ...
        for (int i = 1; i < modelTrain.length; i++) {
            Minecraft.getMinecraft().renderEngine.bindTexture(trainTex[i]);
            modelTrain[i].render(minecart, 0.0F, 0.0F, -0.1F, 0.0F, 0.0F, 0.0625F);
        }
        GL11.glPopMatrix();
