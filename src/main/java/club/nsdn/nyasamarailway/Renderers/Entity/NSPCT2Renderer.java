 ...
 */
public class NSPCT2Renderer extends RenderMinecart {
    private ResourceLocation trainTex[];
    public ModelBase modelTrain[];

    public NSPCT2Renderer(ModelBase model[], String texturePath[]) {
        super();
        trainTex = new ResourceLocation[texturePath.length];
        for (int i  = 0; i < texturePath.length; i++) {
            trainTex[i] = new ResourceLocation("nyasamarailway", texturePath[i]);
        }
        modelTrain = model;
    }
    ...
    public void doRender(EntityMinecart minecart, double x, double y, double z, float Yaw, float p_doRender_9_) {
        ...
        GL11.glPushMatrix();
        GL11.glTranslatef(0.0F, 0.0F, 1.5F * 8);
        modelTrain[0].render(minecart, 0.0F, 0.0F, -0.1F, 0.0F, 0.0F, 0.0625F);
        GL11.glTranslatef(0.0F, 0.0F, 1.5F * (-16));
        modelTrain[0].render(minecart, 0.0F, 0.0F, -0.1F, 0.0F, 0.0F, 0.0625F);
        GL11.glTranslatef(0.0F, 0.0F, 1.5F * 8);

        Minecraft.getMinecraft().renderEngine.bindTexture(trainTex[1]);

        modelTrain[1].render(minecart, 0.0F, 0.0F, -0.1F, 0.0F, 0.0F, 0.0625F);
        for (int i = 1; i <= 8; i++) {
            GL11.glPushMatrix();
            GL11.glTranslatef(0.0F, 0.0F, 1.5F * i);
            modelTrain[1].render(minecart, 0.0F, 0.0F, -0.1F, 0.0F, 0.0F, 0.0625F);
            GL11.glPopMatrix();
