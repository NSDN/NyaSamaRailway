 ...
 */
public class NSRM2G extends AbsTrainBase {

    private static final DataParameter<Boolean> DOOR_STATE_L = EntityDataManager.createKey(NSRM2G.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> DOOR_STATE_R = EntityDataManager.createKey(NSRM2G.class, DataSerializers.BOOLEAN);
    ...
    public NSRM2G(World world) {
        super(world);
        setSize(3.0F, 3.5F);
    }

    public NSRM2G(World world, double x, double y, double z) {
        super(world, x, y, z);
        setSize(3.0F, 3.5F);
    }
    ...
    public void updatePassenger(@Nonnull Entity entity) {
        CartUtil.updatePassenger12CG(this, entity);
    }
    ...
    protected void removePassenger(Entity entity) {
        ...
        if (getStateLeft(invert))
            ...
            if (world.getBlockState(pos.down().offset(facing.rotateYCCW(), 2)).getBlock() instanceof BlockPlatform)
                pos = pos.offset(facing.rotateYCCW(), 3);
            else if (world.getBlockState(pos.down().offset(facing.rotateY(), 2)).getBlock() instanceof BlockPlatform)
                pos = pos.offset(facing.rotateY(), 3);
            else {
    ...
    protected boolean canFitPassenger(Entity entity) {
        ...
        if (getShield(pos.offset(facing.rotateYCCW(), 2), facing) != null) return false;
        if (getShield(pos.offset(facing.rotateY(), 2), facing) != null) return false;
