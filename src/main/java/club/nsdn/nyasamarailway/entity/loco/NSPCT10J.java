 ...
 */
public class NSPCT10J extends AbsLimLoco implements IHighSpeedCart, IInspectionCart {

    public boolean isHighSpeedMode = false;
    private static final DataParameter<Boolean> HIGH = EntityDataManager.createKey(NSPCT10J.class, DataSerializers.BOOLEAN);
    ...
    public void modifyHighSpeedMode(EntityPlayer player) {
        ...
        Util.say(player, "info.nspc108j.mode", String.valueOf(getHighSpeedMode()).toUpperCase());
    }
    ...
    public void doMotion(TrainPacket packet, EntityMinecart cart) {
        if (getHighSpeedMode())
            TrainController.doMotionWithAirHighEx(packet, cart);
        else {
